...
[backend/inference/pipeline.py](backend/inference/pipeline.py) `process_video` is the heavy loop. **Everything runs in native frame coordinates** — YOLO detection (explicit `imgsz`, default 640) every Nth frame, tracking, ROI tests, and classifier crops; `resize_dim` only sets the annotated-output canvas, drawn with scaled coordinates. Tracker backends: Ultralytics-fused `bytetrack` (default; measured 94.7% macro counting accuracy vs centroid's 88.4%) / `botsort` via `model.track(persist=True)`, plus `centroid` and `deepsort` fallbacks. The fused path uses a **separate cached YOLO instance** (tracking callbacks would contaminate plain detection) and resets tracker state per video via `reset()` — never by deleting `predictor.trackers`, which makes `model.track` re-register duplicate callbacks and silently corrupts association. `TVA_PIPELINE_TRACK_CONF` bounds the detections the tracker sees (lower bridges occlusions, floods dense scenes; the counting-grade confidence filter re-applies on emitted boxes). Counting is **entry-based** ([counting.py](backend/inference/counting.py) `RoiEntryCounter`, unit-tested): a vehicle is counted the moment its tracked centroid enters the ROI polygon (held under `Unclassified`), then MobileNetV3 **multi-frame voting** ([classification_voting.py](backend/inference/classification_voting.py)) resolves the label and the count is *moved* to the voted class — counting never depends on classification readiness, matching the original methodology report (`traffic-video-analytics-part-of-original-report.pdf`, §3.5), the authoritative design + evaluation reference. Tracks must survive `min_hits` detections before they can be counted. **Train/serve parity is locked by [tests/test_classifier_parity.py](tests/test_classifier_parity.py)**: training fed raw [0,255] RGB (no `preprocess_input` — it is an identity function in TF 2.19) with a square stretch; do not "fix" the serving preprocessing without retraining. Counts and `time_series` are **cumulative totals**, not per-interval flows — the GPT prompt and analytics depend on this (per-class values may dip when a pending label resolves; the total never does). The result also carries `classification_confidence` (per-label mean vote confidence).
